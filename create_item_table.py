...
ACTION_201603_EXTRA_FILE = "E:/Study/pystudy/JData-master/data/JData_Action_201603_extra.csv"
...
ITEM_TABLE_FILE = "E:/Study/pystudy/JData-master/data/item_table.csv"


def get_from_jdata_product():   #获得评论数据
    df_item = pd.read_csv(PRODUCT_FILE, header=0)
    return df_item


# apply type count    #类型数计算
...
def get_from_action_data(fname, chunk_size=100000):
    ...
    while loop:
        try:
            chunk = reader.get_chunk(chunk_size)[["sku_id", "type"]]     #获取商品id和type
            chunks.append(chunk)
...
def get_from_jdata_comment():   #获取商品评论数据
    df_cmt = pd.read_csv(COMMENT_FILE, header=0)
    df_cmt['dt'] = pd.to_datetime(df_cmt['dt'])   #调用pd.to_datetime() 将时间转化为标准datetime
